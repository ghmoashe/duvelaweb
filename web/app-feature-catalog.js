(function () {
  function createCatalogFeature(ctx) {
    ...
    function learnerHomeV3(liveItems, scheduledItems) {
      ...
      const video=state.videos[0]||{},events=(state.events||[]).slice(0,8),courses=(state.courses||[]).slice(0,4),live=(liveItems||[]).slice(0,4);
      ...
      const eventCard=item=>'<article class="lh-event" data-home-event data-date-kind="'+dateKind(item)+'" data-language="'+esc(String(item.language||''))+'" data-level="'+esc(String(item.level||''))+'" data-format="'+(item.is_online?'online':'offline')+'" data-home-card="'+esc([item.title,item.city,item.language,item.level,item.description].filter(Boolean).join(' ').toLowerCase())+'">'+
      ...
      const mini=(item,kind)=>'<a class="lh-mini" href="#'+kind+'" data-go="'+kind+'" data-home-card="'+esc([item.title,item.description,item.teacher_name,item.level].filter(Boolean).join(' ').toLowerCase())+'"><span class="lh-mini-icon">'+(kind==='courses'?'▣':'◉')+'</span><span><b>'+esc(item.title||item.teacher_name||tr('Duvela lesson','Урок Duvela'))+'</b><small>'+esc(item.level||item.description||tr('Open details','Открыть подробнее'))+'</small></span></a>';
      host.innerHTML='<div class="learner-home">'+
        ...
        '<nav class="lh-primary-tabs"><a class="active" href="#events" data-go="events">▦ '+esc(tr('Events','События'))+'</a><a href="#courses" data-go="courses">▣ '+esc(tr('Courses','Курсы'))+'</a><a href="#live" data-go="live">◉ LIVE</a><a href="#workspace" data-go="workspace">♕ '+esc(tr('Challenges','Челленджи'))+'</a></nav><div class="lh-filter-tabs">'+[['all',tr('All','Все')],['upcoming',tr('Upcoming','Предстоящие')],['today',tr('Today','Сегодня')],['weekend',tr('Weekend','Выходные')]].map((x,i)=>'<button data-home-filter="'+x[0]+'" class="'+(i?'':'active')+'">'+esc(x[1])+'</button>').join('')+'</div>'+
        ...
        '<section class="lh-section"><div class="lh-heading"><h2>'+esc(tr('Recommended events','Рекомендованные события'))+'</h2><a href="#events" data-go="events">'+esc(tr('See all','Смотреть все'))+'</a></div><div class="lh-event-grid">'+(events.length?events.map(eventCard).join(''):'<div class="lh-empty">'+esc(tr('New events will appear here.','Новые события появятся здесь.'))+'</div>')+'</div></section><section class="lh-section"><div class="lh-heading"><h2>'+esc(tr('Courses and LIVE','Курсы и LIVE'))+'</h2></div><div class="lh-mini-grid">'+(courses.map(x=>mini(x,'courses')).concat(live.map(x=>mini(x,'live'))).join('')||'<div class="lh-empty">'+esc(tr('Recommendations are being prepared.','Рекомендации готовятся.'))+'</div>')+'</div></section><div class="lh-no-results" id="learnerHomeEmpty" hidden>'+esc(tr('Nothing matched your filters.','По фильтрам ничего не найдено.'))+'</div>'+
      ...
      let dateFilter='all';const apply=()=>{const q=(host.querySelector('#learnerHomeSearch').value||'').trim().toLowerCase(),lang=host.querySelector('#lhLanguage').value.toLowerCase(),level=host.querySelector('#lhLevel').value,format=host.querySelector('#lhFormat').value;let visible=0;host.querySelectorAll('[data-home-card]').forEach(card=>{let show=!q||card.dataset.homeCard.includes(q);if(card.matches('[data-home-event]'))show=show&&(dateFilter==='all'||card.dataset.dateKind===dateFilter)&&(!lang||card.dataset.language.toLowerCase()===lang)&&(!level||card.dataset.level===level)&&(!format||card.dataset.format===format);card.hidden=!show;if(show)visible++;});host.querySelector('#learnerHomeEmpty').hidden=Boolean(visible);};
